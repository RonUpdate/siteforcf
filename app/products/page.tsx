...
// Cache the products and categories fetching
const getProductsCached = cache(async (page = 1) => {
  try {
    ...
    return { products: data as (Product & { category: Category; images: any[] })[], totalPages }
  } catch (error) {
    console.error("Error fetching products:", error)
    return { products: [], totalPages: 1 }
  }
...
async function ProductsList({ page }: { page: number }) {
  const { products, totalPages } = await getProductsCached(page)
